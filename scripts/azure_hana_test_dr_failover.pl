...
sub runGetVolumeLocations
{
  ...
  my $strSSHCmd = "volume show -volume *".$strHANAInstance."* -volume !*clone* -fields volume";
...
print color('reset');
    ...
    logMsg( $LOG_INFO, "Collecting set of snapshots for each volume hosting HANA matching pattern *$strHANAInstance* ..." );
        ...
        my $strSSHDiagCmd = "snapshot show -volume $volName -snapshot !*snapmirror* -sort-by create-time -snapmirror-label hourly|5min|15min -fields snapshot";
...
#          for my $j (1 .. $#{$aref} ) {
          logMsg($LOG_INFO,"Snapshot:".$snapshotLocations[$i][$j]);
              ...
              if ($snapshotLocations[$i][$j] =~ /\.0/ ) {
                  $snapshotCloneList[$snapshotCount][0] = $snapshotLocations[$i][0];
                  $snapshotCloneList[$snapshotCount][1] =  $snapshotLocations[$i][$j];
                  $snapshotCount++;
              }
...
{
print color('bold cyan');
logMsg($LOG_INFO, "**********************Cloning Snapshots by Volume**********************");
print color('reset');
         for my $i (0 .. $#snapshotCloneList) {

            my $strSSHCmd = "volume clone create -flexclone ".$snapshotCloneList[$i][0]."_".$date."_clone -type RW -parent-volume ".$snapshotCloneList[$i][0]." -junction-active true -foreground true -parent-snapshot ".$snapshotCloneList[$i][1]." -junction-path /".$snapshotCloneList[$i][0]."_".$date."_clone";
            my @out = runSSHCmd( $strSSHCmd );
            if ( $? ne 0 ) {
          		logMsg( $LOG_WARN, "Running '" . $strSSHCmd . "' failed: $?" );
              logMsg( $LOG_WARN, "Snapshot cloned for ".$snapshotCloneList[$i][0]."  failed.  Please reach out to MS Operations for assistance");
              exit;
            } else {
          		logMsg( $LOG_INFO, "Snapshot cloned for volume ".$snapshotCloneList[$i][0]." completed successfully." );
          	}
        }
        print color('bold green');
        logMsg($LOG_INFO, "**********************All Data volumes restored sucessfully**********************");
        print color('reset');
        my $strSSHCmd = "volume clone create -flexclone ".$logBackupsVolume."_".$date."_clone -type RW -parent-volume ".$logBackupsVolume." -junction-active true -foreground true -parent-snapshot ".$logBackupsSnapshot." -junction-path /".$logBackupsVolume."_".$date."_clone";
        my @out = runSSHCmd( $strSSHCmd );
        if ( $? ne 0 ) {
          logMsg( $LOG_WARN, "Running '" . $strSSHCmd . "' failed: $?" );
          logMsg( $LOG_WARN, "Snapshot cloned for ".$logBackupsVolume."  failed.  Please reach out to MS Operations for assistance");
          exit;
        } else {
          logMsg( $LOG_INFO, "Snapshot cloned for volume ".$logBackupsVolume." completed successfully." );
        }
}
...
sub runDisplayMountPoints {
    ...
    for my $i (0 .. $#addressLocations) {
                ...
                if ($addressLocations[$i][0] =~ "data") {
                    ...
                    logMsg($LOG_INFO,$addressLocations[$i][1].":/".$addressLocations[$i][0]."_".$date."_clone   /hana/data/".$strHANASID."/".$mountNode."   ".$strMountOptions);
                ...
                if ($addressLocations[$i][0] =~ "log_backups") {
                    ...
                    logMsg($LOG_INFO,$addressLocations[$i][1].":/".$addressLocations[$i][0]."_".$date."_clone   /hana/logbackups/".$strHANASID."   ".$strMountOptions);
                }
...
print color('reset');
         for my $i (0 .. $#replicationList) {
            ...
            my $strSSHCmd = "snapmirror resume -destination-vserver * -destination-volume ".$replicationList[$i];
            my @out = runSSHCmd( $strSSHCmd );
            if ( $? ne 0 ) {
          		logMsg( $LOG_WARN, "Running '" . $strSSHCmd . "' failed: $?" );
              logMsg( $LOG_WARN, "The replication relationship for ".$replicationList[$i]." could not be resumed.  Please reach out to MS Operations for assistance");
              exit;
            } else {
          		logMsg( $LOG_INFO, "Relationship resume for volume ".$replicationList[$i]." completed successfully." );
          	}
...
#resumes temp suspension of snapmirror relationship
runResumeSnapMirror();
