...
sub runSSHDiagCmd {
    ...
    #logMsg( $LOG_WARN, "Running NetApp diagnostics privilege commands." );
    my @sshcmd_output;
  ...
  RUNCOMMAND: {                                  # start of do..until block to allow 'last' to exit block.
    ...
    }    # end of RUNCOMMAND block

    return ( $sshcmd_return_code, @sshcmd_output );
}
...
#
### Name: runCheckHSRStatus()
### Func: Check if we are running on HSR and if both nodes are up,   exit without a snapshot!
###
sub runCheckHSRStatus {
    print color('bold cyan');
    logMsg( $LOG_CRIT, "**********************Checking HSR status**********************" );
    print color('reset');
    ...
    if (@HSRNodes) {
        foreach my $HSRNode (@HSRNodes) {
            logMsg( $LOG_CRIT, "Checking if HSRNode '" . $HSRNode . "' is up." );

            # SQL/MDX access port for standard access to the system database of a multitenant system.
            # SAP HANA Platform indexserver default=30013, range=30013–39913,
            #  rule=3NN13,  where NN is the instance number.
            # ref:  https://help.sap.com/viewer/ports
            my $HANAPort = "3" . $strHANANumInstance . "13";    #ref https://help.sap.com/viewer/ports
            $| = 1;                                             # flush the buffer
            my $socketConnection = new IO::Socket::INET(
                PeerHost => $HSRNode,
                PeerPort => $HANAPort,
                Proto    => 'tcp',
                Timeout  => 1
            ...
            logMsg( $LOG_CRIT, "Checking connection to HANA Server " . $HSRNode . ":" . $HANAPort );
            if ($socketConnection) {
                logMsg( $LOG_CRIT, "HSRNode '" . $HSRNode . "' is up." );

                # now checking if the other node is a secondary read-only configuration.
                # first we need the virtual schema name for the Site Replication virtual view.
                logMsg( $LOG_CRIT, "Checking possible system replication configuration for  '" . $HSRNode . "'." );
                my $sqlselect = q{select SCHEMA_NAME from SCHEMAS where SCHEMA_OWNER='_SYS_SR' and HAS_PRIVILEGES = 'TRUE';};
                my $sqlcmd    = qq{hdbsql -jaxC -n $strPrimaryHANAServerIPAddress -i $strHANANumInstance -U $strHANAAdmin "$sqlselect"};
                my $sr_schema = `$sqlcmd`;
                chomp($sr_schema);

                if ( not $sr_schema eq "" ) {    # we have a system replication definition.
                    logMsg( $LOG_CRIT, "System Replication schema definition is '" . $sr_schema . "'." );

                    # do we have a logreplay_readaccess setup
                    $sqlselect = q{select VALUE from \"} . $sr_schema . q{\".\"M_INIFILE_CONTENTS\" where (KEY = 'operation_mode' and VALUE = 'logreplay_readaccess');};
                    $sqlcmd    = qq{hdbsql -jaxC -n $strPrimaryHANAServerIPAddress -i $strHANANumInstance -U $strHANAAdmin "$sqlselect"};

                    #print "$sqlcmd\n";
                    my $operation_mode = `$sqlcmd`;
                    chomp($operation_mode);

                    #print "$operation_mode\n";

                    if ( $operation_mode eq "logreplay_readaccess" ) {
                        logMsg( $LOG_CRIT, "This site has a Secondary Node configured in operation_mode = '" . $operation_mode . "'." );

                        # are we the same hostname as the system_replication_site_masters
                        my $HANA_hostname = `hostname`;
                        chomp($HANA_hostname);
                        $sqlselect = q{select VALUE from \"} . $sr_schema . q{\".\"M_INIFILE_CONTENTS\" where (SECTION = 'system_replication_site_masters' and VALUE like '%} . $HANA_hostname . q{%');};
                        $sqlcmd    = qq{hdbsql -jaxC -n $strPrimaryHANAServerIPAddress -i $strHANANumInstance -U $strHANAAdmin "$sqlselect"};
                        my $system_replication_site_masters = `$sqlcmd`;
                        chomp($system_replication_site_masters);
                        if ( $system_replication_site_masters eq "" ) {
                            logMsg( $LOG_CRIT, "HSR node $HANA_hostname is not the master and the master is running, cannot proceed with snapshot!" );
                            runExit($exitWarn);
                        }
                        else {
                            logMsg( $LOG_CRIT, "HSR node $HANA_hostname is the master, proceed with snapshot!" );
                        }
                    }
                    else {
                        # operation_mode needs to be logreplay_readaccess if we can connect to other node, do not snapshot!
                        logMsg( $LOG_CRIT, "System Replication operation mode not 'logreplay_readaccess', cannot proceed with snapshot!" );
                        logMsg( $LOG_CRIT, "operation_mode = '" . $operation_mode . "'" );
                        runExit($exitWarn);
                    }

                }
                else {
                    # system replication schema is empty, could be a cluster issue, do not snapshot!
                    logMsg( $LOG_CRIT, "HSR node $HSRNode appears to be running in unknown state, cannot proceed with snapshot!" );
                    runExit($exitWarn);
                }
            }
            else {
                # can't connect to HSR Node, so if this script runs on other node it won't work.  Ok to snapshot!
                logMsg( $LOG_CRIT, "HSR node $HSRNode does not seem to be running, continuing with snapshot!" );
            }
        }
    }
    else {
        print color('bold green');
        logMsg( $LOG_CRIT, "System not in HSR Configuration" );
        print color('reset');
    }
}
...
sub runCheckIfSnapshotCreated {
    my $volName = shift;
    logMsg( $LOG_INFO, "Checking if a snapshot was created for volume $volName..." );
...
sub runCreateStorageSnapshot {
    ...
    $hostname =~ s/\-/_/g;      # convert hyphen to underscore as Volume Names can't have a hyphen
...
sub runGetSnapshotsByVolume {
    ...
    foreach my $volName (@volLocations) {
        ...
        #if ( $? ne 0 ) {
        my ( $ret_code, @out ) = runSSHDiagCmd($strSSHDiagCmd);
        if ( $ret_code ne 0 ) {
        ...
        foreach my $k ( 0 ... $count ) {
            ...
            if ( $listnum >= 3 ) {
                ...
                # record-owner field used to determine if a snapmirror clone exists
                # we must not delete any snapshots where the record-owner contains snapmirror.
                my $strRecordOwner = $strSubArr[ $#strSubArr - 1 ];    # record-owner field.
                my $strSub         = $strSubArr[ $#strSubArr - 2 ];    # snapshot field.
...
sub runCheckSnapshotLocked {
    ...
    # record-owner field used to determine if a snapmirror clone exists
    # we must not delete any snapshots where the record-owner contains snapmirror.
    my $strRecordOwner = $snapshotLocations[$i][$j][1];    # record-owner field.
    if ( $strRecordOwner =~ m/snapmirror/ ) {
        print color('bold cyan');
        logMsg( $LOG_INFO, "Snapshot $snapshotName of volume $volName has record owner of $strRecordOwner" );
        print color('reset');
        return 1;
    }
    else {
...
if ( $strBackupType eq "hana" or $strBackupType eq "logs" ) {
    ...
    for my $i ( 0 .. $numSID ) {
          ...
          )
        {
            ...
            # be more flexible with the frequency parameter to avoid customer confusion
            # allowed values are number+min (e.g. 3min, 5min, 9min, 15min, 20min, 99min, etc).
            if ( not $strSnapshotCustomerLabel =~ /^[0-9]*min$/ ) {

                #current accepted snapmirror labels (frequency) are 15min for data, 3min for log backups
                logMsg( $LOG_WARN, "A frequency of $strSnapshotCustomerLabel is not a recommended frequency." );
                logMsg( $LOG_WARN, "For HANA backups and boot backups, please use 15min" );
                logMsg( $LOG_WARN, "For log backups, please use 3min" );
                runExit($exitWarn);
            }

            # for backwards compatibility, set defaults for the snapmirror label
            # which are used in the DR failover scripts to list snapshots.
            $strSnapshotCustomerLabel = "15min" if $strBackupType eq "hana";
            $strSnapshotCustomerLabel = "3min"  if $strBackupType eq "logs";
            logMsg( $LOG_CRIT, "Backup type is '$strBackupType', therefore setting snapmirror-label to '$strSnapshotCustomerLabel'." );
