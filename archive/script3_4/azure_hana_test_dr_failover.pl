...
# Global Tunables
#
my $weeksCloneKept = 4;    # how long will we keep the FlexClones for.
my $cloneExpiryDate;
my $contactEmail;
my $email_ok;
...
my $logCount = 0;                     #counts number of lob backups volumes that match SID, used in two functions to ensure mount points displayed correctly.
...
sub logMsg {
    ...
    #$LOG_INPUT - this does not include the timestamp prefix for log entries and screen displays.
    if ( $errValue eq 4 ) {
        $str .= "$msgString\n";
        print $str;
        push( @arrOutputLines, $str );
...
sub runVerifySnapshotsExist {
    ...
    if ( $HSR eq 0 ) {
        for my $i ( 0 .. $#dataCloneList ) {
            my $volumeNode   = $dataCloneList[$i][0];
            my $snapshotNode = $dataCloneList[$i][1];
            print color('bold magenta');
            logMsg( $LOG_CRIT, "Adding volume $volumeNode to list for recovery." );
            print color('reset');
            my @tempArr;
            $tempArr[0][0] = $volumeNode;
            $tempArr[0][1] = $snapshotNode;
            push( @snapshotCloneList, @tempArr );
        }

        if ( $snapshotCount eq $dataCount ) {
            print color('bold green');
        ...
        for my $i ( 0 .. $#logBackupsList ) {
            my $volumeNode   = $logBackupsList[$i][0];
            my $snapshotNode = $logBackupsList[$i][1];
            print color('bold magenta');
            logMsg( $LOG_CRIT, "Adding volume $volumeNode to list for recovery." );
            print color('reset');
            my @tempArr;
        ...
        if ( $snapshotLogCount eq $logCount ) {
            print color('bold green');
    ...
    if ( $HSR eq 1 ) {
        ...
        if ( $logCount eq 2 ) {
            logMsg( $LOG_CRIT, "Checking for HSR log backups volume snapshot" );
                ...
                and $snapshotNode2 ne "" )
            {
                ...
                # volumeNode1
                print color('bold magenta');
                logMsg( $LOG_CRIT, "Adding volume $volumeNode1 to list for cloning." );
                print color('reset');
                my @tempArr;
                @tempArr       = ();
                $tempArr[0][0] = $volumeNode1;
                ...
                # volumeNode2
                print color('bold magenta');
                logMsg( $LOG_CRIT, "Adding volume $volumeNode2 to list for cloning." );
                print color('reset');
                @tempArr       = ();
                $tempArr[0][0] = $volumeNode2;
                $tempArr[0][1] = $snapshotNode2;
...
sub runUpdateSnapMirrorRelationship {
    ...
    if ( $state eq "Snapmirrored" ) {
        ...
        $status = "Transferring";
        while ( $status ne "Idle" ) {
            Time::HiRes::sleep( 5 + rand(5) );
            logMsg( $LOG_CRIT, "Volume $volName currently $status. Waiting until finished. Please do not cancel script." );
            my $strSSHCmd = "snapmirror show -destination-volume $volName -fields status";
            my @out       = runSSHCmd($strSSHCmd);
            if ( $? ne 0 ) {
                logMsg( $LOG_WARN, "Running '" . $strSSHCmd . "' failed: $?" );
...
#
# Name: runCloneSnapshot
# Func: creates a clone the latest snapshot in the data volumes and log backups volume
#

sub runCloneSnapshot {
    print color('bold cyan');
    logMsg( $LOG_CRIT, "**********************Cloning Snapshots by Volume**********************" );
    logMsg( $LOG_CRIT, $#snapshotCloneList . " snapshots to clone" );
    print color('reset');
    ...
    for my $i ( 0 .. $#snapshotCloneList ) {
        ...
        if ( $? ne 0 ) {
            ...
            logMsg( $LOG_CRIT, "Snapshot cloned for volume " . $volName . " completed successfully." );
            push( @arrClones,     $strClone );
            push( @displayMounts, $strClone );
        }
    }
...
sub runDisplayRecoverySteps {
    ...
    print color('bold cyan');
    logMsg( $LOG_CRIT, "**********  HANA Test DR Recovery Steps  **********" );
    print color('reset');
    logMsg( $LOG_INPUT, "1. Ensure ALL the target mount points exist (e.g. mkdir /hana/logbackups/H99_SOURCE)." );
    logMsg( $LOG_INPUT, "2. Add Mount Point Details into /etc/fstab of DR Server." );
    logMsg( $LOG_INPUT, "3. Mount newly added filesystems." );
    logMsg( $LOG_INPUT, "4. Perform HANA Snapshot Recovery using HANA Studio." );
    print color('bold cyan');
    logMsg( $LOG_INPUT, "****************************************************************************************" );
    print color('reset');
    logMsg( $LOG_INPUT, " These snapshot copies (clones) are kept for $weeksCloneKept weeks before being automatically removed." );
    logMsg( $LOG_INPUT, " Please contact Microsoft Operations to delete them earlier." );
    print color('bold cyan');
    logMsg( $LOG_CRIT, "***************************************************" );
    print color('reset');
...
##### --------------------- MAIN CODE --------------------- #####
logMsg( $LOG_CRIT, "Executing Azure HANA Test DR Failover Script, Version $version" );
my $SCRIPT_NAME = $0;
...
logMsg( $LOG_INPUT, "Proceed with script: " . $inputProceed );

my $strHANASIDInput = "Please enter either the HANA SID you wish to restore:   ";
print $strHANASIDInput;
my $inputHANASID = <STDIN>;
$inputHANASID =~ s/[\n\r\f\t]//g;
$strHANASID = lc $inputHANASID;
logMsg( $LOG_INPUT, $strHANASIDInput . ": " . $strHANASID );

# we want to get a contact email to get in touch when deleting clones.
do {
    print "Please enter a contact email address for Microsoft Operations to confirm ok to delete clones:  ";
    $contactEmail = <STDIN>;
    chomp $contactEmail;
    my $username = qr/[a-z0-9_+]([a-z0-9_+.]*[a-z0-9_+])?/;
    my $domain   = qr/[a-z0-9.-]+\.[a-z0-9.-]+/;
    $email_ok = $contactEmail =~ /^$username\@$domain$/;
} until ($email_ok);
logMsg( $LOG_INFO, "Contact email -> " . $contactEmail );
...
#identifies HSR volumes
#runIdentifyHSRVolumes();
...
#displays necessary recover steps
runDisplayRecoverySteps();
