...
my $logCount = 0;                     #counts number of lob backups volumes that match SID, used in two functions.
...
sub logMsg {
    ...
    #$LOG_INPUT - don't prefix with timestamp as it clutters the input.
    if ( $errValue eq 4 ) {
        $str .= "$msgString\n";
        print "$str";
        push( @arrOutputLines, $str );
    ...
    for my $i ( 0 .. $#snapshotLocations ) {
        ...
        if ( $volName =~ /log_backups/ ) {
            ...
            if (   $snapshotLocations[$i][$j] =~ /\.0/
                or $snapshotLocations[$i][$j] =~ /\.recent/ )
            {
    ...
    if ( $HSR eq 1 ) {
        ...
        if ( $logCount eq 2 ) {
                ...
                and $snapshotNode2 ne "" )
            {
                ...
                # volumeNode1
                print color('bold magenta');
                logMsg( $LOG_CRIT, "Adding volume $volumeNode1 to list for recovery." );
                print color('reset');
                my @tempArr;
                @tempArr       = ();
                $tempArr[0][0] = $volumeNode1;
                ...
                # volumeNode2
                print color('bold magenta');
                logMsg( $LOG_CRIT, "Adding volume $volumeNode2 to list for recovery." );
                print color('reset');
                @tempArr       = ();
                $tempArr[0][0] = $volumeNode2;
                $tempArr[0][1] = $snapshotNode2;
...
sub runUpdateSnapMirrorRelationship {
    ...
    if ( $state eq "Snapmirrored" ) {
        if ( $status eq "Quiescing" or $status eq "Quiesced" ) {
            logMsg( $LOG_CRIT, "Volume $volName currently $status." );
            return;
        }
...
sub runPrintFile {
    ...
    if ( defined($strHANASID) ) {
        $outputFilename = "FullDR.$strHANASID.$date.txt";
    }
    else {
        $outputFilename = "FullDR.$date.txt";
    }
...
do {
    my $inputPrompt = "Please enter (yes/no): ";
    print $inputPrompt;
    $inputProceed = <STDIN>;
    $inputProceed =~ s/[\n\r\f\t]//g;
    logMsg( $LOG_INPUT, "$inputPrompt $inputProceed" );
