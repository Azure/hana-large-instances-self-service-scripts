...
print color('reset');
         ...
         for my $i (0 .. $#snapshotCloneList) {
            ...
            my $strSSHCmd = "volume clone create -flexclone $strClone -type RW -parent-volume ".$volName." -junction-active true -foreground true -parent-snapshot ".$snapshotName." -junction-path /".$strClone;
            my @out = runSSHCmd( $strSSHCmd );
            if ( $? ne 0 ) {
          		logMsg( $LOG_WARN, "Running '" . $strSSHCmd . "' failed: $?" );
...
my $inputProceed;
my $strHello = "This script is designed for those customers who have previously installed the Production HANA instance in the Disaster Recovery Location either as a stand-alone instance or as part of a multi-purpose environment. This script is intended to allow the customer to simulate a Disaster Recovery failover without actually requiring a failover and subsequent failback.  This script will clone the most recent snapshot for both the Data and Log Backups filesystems.  Any other restore points must be handled by Microsoft Operations.  Please enter the HANA <SID> you wish to restore. This script must be executed from the Disaster Recovery location otherwise unintended actions may occur.  As part of the script process, a clone is created of the necessary Disaster Recovery volumes for Data and Log Backups volumes.  This clone is kept for 4 weeks before it is automatically removed.  \n";
$strHello =~ s/([^\n]{0,120})(?:\b\s*|\n)/$1\n/gio;
