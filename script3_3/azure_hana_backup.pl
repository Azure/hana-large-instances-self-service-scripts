...
# Date 31-May-2018
my $version = "3.3.2";    #current version number of script
...
my $verbose = 0;
my $strBackupType =
  $ARGV[0];    #type of backup deployed. Options include hana, logs, and boot.
...
my @arrCustomerDetails
  ;   #array that keeps track of all inputs on the HANABackupCustomerDetails.txt
my $strHANABackupID;    #Backup ID number created during HANA backup process.
my $strPrimaryHANAServerName
  ;    #Customer provided IP Address or Qualified Name of Primay HANA Server.
my $strPrimaryHANAServerIPAddress
  ;    #Customer provided IP address of Primary HANA Server
...
my $numKeep
  ; #Customer provided retention number for snapshots by customer provided by snapshot prefix.
my $strSnapshotPrefix
  ; #The customer entered snapshot prefix that precedes the date in the naming convention for the snapshot
my $strSnapshotCustomerLabel
  ; #Label provided by customer to snapshot that determines how SnapVault and SnapLock will handle snapshots.
...
my $strHANANumInstance
  ; #the two digit HANA instance number (e.g. 00) the customer uses when installing HANA SIDs
my $strHANAAdmin
  ;    #Hdbuserstore key customer sets for paswordless access to hdbsql
my $strOSBackupType = "none"
  ;    #Input command when customer selects boot backup between Type1 and TypeII
my $cloneSnapshot =
  0;    #keeps track of whether any snapshots in production are cloned in DR
my $numSnapshotCount = 0
  ; #Count of number of snapshots that match snapshot prefix versus total number of snapshots in volume
my $intMDC
  ; #Boolean for determining whether MDC environment is detected. 1 - Yes, 0 - No
my $sshCmd = '/usr/bin/ssh';    #typical location of ssh on SID
my $strHANASID
  ; #The customer entered HANA SID for each iteration of SID entered with HANABackupCustomerDetails.txt
my $hanaSnapshotSuccess = qq('Storage snapshot successful');

#required string for successful closure of HANA snapshot
my $strHANAIDRequestString =
"select BACKUP_ID from M_BACKUP_CATALOG where ENTRY_TYPE_NAME = 'data snapshot' and STATE_NAME = 'prepared'";

#HDBSQL command necessary to obtain HANA Backup ID after creating HANA Snapshot
my $strHANAStatusCmdV1
  ; #generated command for HANA Version 1 to test access by requesting HANA DB status
my $strHANAStatusCmdV2
  ; #generated command for HANA Version 2 to test access by requesting HANA DB status
my $strHANACreateSnapCmdV1
  ;  #generated command for HANA Version 1 to create HANA snapshot within hdbsql
my $strHANACreateSnapCmdV2
  ;  #generated command for HANA Version 2 to create HANA snapshot within hdbsql
my $strHANABackupIDRequestV1
  ; #command for HANA Version 1 to obtain HANA Backup ID for newly created HANA snapshot
my $strHANABackupIDRequestV2
  ; #command for HANA Version 2 to obtain HANA Backup ID for newly created HANA snapshot
my $strHANACloseCmdV1
  ;   #generated command for HANA Version 1 to close HANA snapshot within hdbsql
my $strHANACloseCmdV2
  ;   #generated command for HANA Version 2 to close HANA snapshot within hdbsql

my $strHANAVersion;    #HANA Major Version Number
my $strHANARevision
  ; #HANA Revision release Number. Currently necessary to determine if HANA 2.0 install suports both MDC and HANA Snapshot

my $outputFilename = "";    #Generated filename for scipt output
my @snapshotLocations
  ;    #arroy of all snapshots for certain volumes that match customer SID.
...
my @snapshotCreated;    #array of volumes where snapshots are created.
    ...
    if ( $fileLines[ $lineNum - 1 ] ) {
        if ( index( $fileLines[ $lineNum - 1 ], $strServerName ) eq -1 ) {
            ...
            logMsg( $LOG_WARN,
                    "Verify line "
                  . $lineNum
                  . " is for the HANA Server Name. Exiting" );
            runExit($exitWarn);
        }
    }
    ...
    if ( $fileLines[ $lineNum - 1 ] ) {
        if ( index( $fileLines[ $lineNum - 1 ], $strHANAIPAddress ) eq -1 ) {
            ...
            logMsg( $LOG_WARN,
                    "Verify line "
                  . $lineNum
                  . " is the HANA Server IP Address. Exiting" );
            runExit($exitWarn);
        }
    }
    ...
    for my $i ( 0 ... $numSID ) {
        ...
        if ( $fileLines[ $lineNum - 1 ] ) {
            if ( index( $fileLines[ $lineNum - 1 ], $string1 ) eq -1 ) {
                ...
                logMsg( $LOG_WARN,
                    "Verify line " . $lineNum . " is correct. Exiting" );
                runExit($exitWarn);
            }
        }
        $j++;
        ...
        if ( $fileLines[ $lineNum - 1 ] ) {
            if ( index( $fileLines[ $lineNum - 1 ], $string2 ) eq -1 ) {
                ...
                logMsg( $LOG_WARN,
                    "Verify line " . $lineNum . " is for SID #$i. Exiting" );
                runExit($exitWarn);
            }
        }
        $j++;
        ...
        if ( $fileLines[ $lineNum - 1 ] ) {
            if ( index( $fileLines[ $lineNum - 1 ], $string4 ) eq -1 ) {
                ...
                logMsg( $LOG_WARN,
                        "Verify line "
                      . $lineNum
                      . " contains the storage backup as provied by Microsoft Operations. Exiting."
                );
                runExit($exitWarn);
            }
        }
        ...
        if ( $fileLines[ $lineNum - 1 ] ) {
            if ( index( $fileLines[ $lineNum - 1 ], $string5 ) eq -1 ) {
                ...
                logMsg( $LOG_WARN,
                        "Verify line "
                      . $lineNum
                      . " contains the Storage IP Address. Exiting." );
                runExit($exitWarn);
            }
        }
        $j++;
        ...
        if ( $fileLines[ $lineNum - 1 ] ) {
            if ( index( $fileLines[ $lineNum - 1 ], $string7 ) eq -1 ) {
                ...
                logMsg( $LOG_WARN,
                        "Verify line "
                      . $lineNum
                      . " contains the HANA instance number. Exiting." );
                runExit($exitWarn);
            }
        }
        $j++;
        ...
        if ( $fileLines[ $lineNum - 1 ] ) {
            if ( index( $fileLines[ $lineNum - 1 ], $string8 ) eq -1 ) {
                ...
                logMsg( $LOG_WARN,
                        "Verify line "
                      . $lineNum
                      . " contains the HDBuserstore Name. Exiting." );
                runExit($exitWarn);
            }
        }
        $j++;
        ...
        if ( $#fileLines >= $lineNum - 1 and $fileLines[ $lineNum - 1 ] ) {
            if ( $fileLines[ $lineNum - 1 ] ne "" ) {
                ...
                logMsg( $LOG_WARN,
                    "Verify line " . $lineNum . " is blank. Exiting." );
                runExit($exitWarn);
            }
        }
    }
...
sub runGetParameterDetails {
    ...
    if ( substr( $fileLines[ $lineNum - 1 ], 0, 1 ) ne "#" ) {
        ...
        logMsg( $LOG_WARN,
            "Cannot skip HANA Server Name. It is a required field" );
        runExit($exitWarn);
    }
    ...
    if ( substr( $fileLines[ $lineNum - 1 ], 0, 1 ) ne "#" ) {
        ...
        logMsg( $LOG_WARN,
            "Cannot skip HANA Server IP Address. It is a required field" );
        runExit($exitWarn);
    }
    if ( $strSnapSplit[1] and $strSnapSplit[1] !~ /^\s*$/ ) {
        $strSnapSplit[1] =~ s/^\s+|\s+$//g;
        ...
        logMsg( $LOG_CRIT,
            "HANA Server IP Address: " . $strPrimaryHANAServerIPAddress );
    }
    ...
    for my $i ( 0 .. $numSID ) {
        ...
        if ( substr( $fileLines[$j], 0, 1 ) ne "#" ) {
            ...
            logMsg( $LOG_CRIT,
                "SID" . ( $i + 1 ) . ": " . $arrCustomerDetails[$i][0] );
        }
        if ( $strSnapSplit[1] and $strSnapSplit[1] !~ /^\s*$/ ) {
            $strSnapSplit[1] =~ s/^\s+|\s+$//g;
        ...
        if ( substr( $fileLines[ $j + 2 ], 0, 1 ) ne "#" ) {
            ...
            logMsg( $LOG_CRIT,
                "Storage Backup Name: " . $arrCustomerDetails[$i][1] );
        }
        if ( $strSnapSplit[1] and $strSnapSplit[1] !~ /^\s*$/ ) {
            $strSnapSplit[1] =~ s/^\s+|\s+$//g;
        ...
        if ( substr( $fileLines[ $j + 3 ], 0, 1 ) ne "#" ) {
            ...
            logMsg( $LOG_CRIT,
                "Storage Backup Name: " . $arrCustomerDetails[$i][2] );
        }
        if ( $strSnapSplit[1] and $strSnapSplit[1] !~ /^\s*$/ ) {
            $strSnapSplit[1] =~ s/^\s+|\s+$//g;
            $arrCustomerDetails[$i][2] = $strSnapSplit[1];
            logMsg( $LOG_CRIT,
                "Storage IP Address: " . $arrCustomerDetails[$i][2] );
        }
        ...
        if ( substr( $fileLines[ $j + 5 ], 0, 1 ) ne "#" ) {
            ...
            logMsg( $LOG_CRIT,
                "HANA Instance Number: " . $arrCustomerDetails[$i][3] );
        }
        if ( $strSnapSplit[1] and $strSnapSplit[1] !~ /^\s*$/ ) {
            $strSnapSplit[1] =~ s/^\s+|\s+$//g;
        ...
        if ( substr( $fileLines[ $j + 6 ], 0, 1 ) ne "#" ) {
            ...
            logMsg( $LOG_CRIT,
                "HANA Instance Number: " . $arrCustomerDetails[$i][4] );
        }
        if ( $strSnapSplit[1] and $strSnapSplit[1] !~ /^\s*$/ ) {
            $strSnapSplit[1] =~ s/^\s+|\s+$//g;
            $arrCustomerDetails[$i][4] = uc $strSnapSplit[1];
            logMsg( $LOG_CRIT,
                "HANA Userstore Name: " . $arrCustomerDetails[$i][4] );
        }
  ...
  NUMSID: for my $i ( 0 ... $numSID ) {
            ...
            and $checkHANAUserstoreName eq 1 )
        {
            ...
            if ( $checkBackupName eq 0 ) {
                logMsg( $LOG_WARN,
                        "Missing Storage Backup Name for SID"
                      . ( $i + 1 )
                      . " Exiting." );
            }
            if ( $checkIPAddress eq 0 ) {
                logMsg( $LOG_WARN,
                        "Missing Storage IP Address for SID"
                      . ( $i + 1 )
                      . " Exiting." );
            }
            if ( $checkHANAInstanceNumber eq 0 ) {
                logMsg( $LOG_WARN,
                        "Missing HANA Instance User Name for SID"
                      . ( $i + 1 )
                      . " Exiting." );
            }
            if ( $checkHANAUserstoreName eq 0 ) {
                logMsg( $LOG_WARN,
                        "Missing HANA Userstore Name for SID"
                      . ( $i + 1 )
                      . " Exiting." );
            }
            runExit($exitWarn);
...
sub runExit {
    ...
    if ( $exitCode eq 1 ) {
        ...
        print color ('bold red');
        logMsg( $LOG_CRIT,
"Command failed. Please check screen output or created logs for errors."
        );
        print color ('reset');
...
sub runSSHCmd {
    ...
    my ($strShellCmd) = @_;
    return (
`"$sshCmd" -l $strUser $strSVM 'set -showseparator ","; $strShellCmd' 2>&1`
    );
}
...
sub runSSHDiagCmd {
    ...
    my ($strShellCmd) = @_;
    return (
`"$sshCmd" -l $strUser $strSVM 'set diag -confirmations off -showseparator ","; $strShellCmd' 2>&1`
    );
}
...
sub runCheckHANAVersion {
    ...
    my $strDirectory2 =
      ...
      . $strHANASIDTemp
      . '/global/hdb/mdc';
    my $strHANAVersionCMD =
        './hdbsql -n '
      . $strPrimaryHANAServerIPAddress . ' -i '
      . $strHANANumInstance . ' -U '
      . $strHANAAdmin
    ...
    if ( -d $strDirectory1 or -d $strDirectory2 ) {
        ...
        print color('bold green');
        logMsg( $LOG_CRIT,
            "Detected non-MDC environment for $strHANASIDTemp." );
        print color('reset');
    }

    logMsg( $LOG_CRIT,
        "Checking HANA Version with command: \"$strHANAVersionCMD\" ..." );
    my @out = runShellCmd($strHANAVersionCMD);
    if ( $? ne 0 ) {
        logMsg( $LOG_WARN, "HANA appears to be down." );
        logMsg( $LOG_WARN,
            "If you feel this is in error, Please contact Microsoft Operations"
        );
        runExit($exitWarn);
    }
    ...
    print color('bold green');
    logMsg( $LOG_CRIT,
        "HANA Version: $strHANAVersion Revision Number: $strHANARevisionLong" );
    print color('reset');
}
...
sub runCheckHANAStatus {
    ...
    if ( $? ne 0 ) {
        ...
        logMsg( $LOG_WARN,
"In an HSR Setup, this script will not function on current secondary node."
        );
        logMsg( $LOG_WARN, "hdbuserstore user command was executed with root" );
        logMsg( $LOG_WARN,
            "Backup user account created in HANA Studio was made under SYSTEM"
        );
        logMsg( $LOG_WARN,
"Backup user account and hdbuserstore user account are case-sensitive"
        );
        logMsg( $LOG_WARN, "The correct host name and port number are used" );
        logMsg( $LOG_WARN,
                "The port number in 3("
              . $strHANANumInstance
              . ")15 [for non-MDC] and 3("
              . $strHANANumInstance
              . ")13 [for MDC] corresponds to instance number of "
              . $strHANANumInstance
              . " when creating hdbuserstore user account" );
        logMsg( $LOG_WARN,
            "******************Exiting Script*******************************" );
        runExit($exitWarn);
    }
...
sub runTYPEIIBackup() {
    ...
    logMsg( $LOG_CRIT,
        "Creating backup of " . $strHANASID . " using rear tool." );
...
"**********************Getting list of volumes that match HANA instance specified**********************"
        ...
        print color('reset');
        logMsg( $LOG_CRIT,
"Collecting set of volumes hosting HANA matching pattern *$strHANASID* ..."
        );
        $strSSHCmd =
            "volume show -volume *"
          . $strHANASID
          . "* -state online -volume !*log_backups* -volume !*log* -type RW -fields volume";
    }
    if ( $strOSBackupType eq "TYPEI" ) {
        print color('bold cyan');
        logMsg( $LOG_CRIT,
"**********************Getting list of volumes of boot volumes**********************"
        );
        print color('reset');
        $strSSHCmd =
          "volume show -volume *boot* -type RW -state online -fields volume";
    }
...
"Collecting set of volumes hosting HANA matching pattern *$strHANASID* ..."
        ...
        $strSSHCmd =
            "volume show -volume *log_backups_"
          . $strHANASID
          . "* -state online -type RW -fields volume";
    }
...
"**********************Getting list of volumes of boot volumes**********************"
        ...
        $strSSHCmd =
            "volume show -volume *"
          . $strPrimaryHANAServerName
          . "_os* -type RW -fields volume";
    }
    ...
    if ( $? ne 0 ) {
        ...
        logMsg( $LOG_WARN,
"Please double check that your HANABackupCustomerDetails sheet includes the correct SID or HANA Server Name depending on backup type"
        );
...
sub runCheckIfSnapshotExists {
    ...
    my $tempSnapName = "";
    logMsg( $LOG_INFO,
        "Checking if snapshot $snapName exists for $volName on SVM $strSVM ..."
    );
    my @strSnapSplit;
    my $listnum = 0;
...
#with the volume found, each snapshot associated with that volume is now examxined
            ...
            for my $j ( 0 .. $#{$aref} ) {
                ...
                if ( $tempSnapName eq $snapName ) {
                    logMsg( $LOG_INFO,
                        "Snapshot $snapName on $volName found." );
...
# Func: Verify if a snapshot exists.  Return 0 if it does not, 1 if it does.
#
sub runCheckIfSnapshotCreated {
    my $volName = shift;
    logMsg( $LOG_INFO,
        "Checking if a snapshot was created for volume $volName..." );

    #checking to make sure volume in $volName exists in the array
    for my $i ( 0 .. $#snapshotCreated ) {
        if ( $volName eq $snapshotCreated[$i] ) {
            logMsg( $LOG_INFO, "$volName found." );
            return ($volName);
        }
    }
    logMsg( $LOG_INFO, "No Snapshot Created for volume $volName." );
    return ("0");
...
sub runRotateSnapshots {
    print color('bold cyan');
    logMsg( $LOG_INFO,
"**********************Rotating snapshot numbering to allow new snapshot**********************"
    );
    print color('reset');
    my $rotateIndex;
...
#determine whether the retention entry from customer is smaller than number of snapshots and sets index
        ...
        my $checkSnapshotCreated = runCheckIfSnapshotCreated($volName);
        if ( $checkSnapshotCreated eq "0" ) {
            next;
        }
        my $checkSnapshotResult;
        ...
        logMsg( $LOG_INFO,
"Rotating snapshots named $strSnapshotPrefix.# on $snapshotLocations[$i][0][0] ..."
        );

        while ( $k >= 0 ) {

            $checkSnapshotResult =
              runCheckIfSnapshotExists( $volName, "$strSnapshotPrefix\.$k" );
            ...
            if ( $checkSnapshotResult ne "0" ) {
                ...
                logMsg( $LOG_INFO,
                      ...
                      . $j );
                my $strSSHCmd =
"volume snapshot rename -volume $volName -snapshot $strSnapSplit[0]\.$strSnapSplit[1]\.$k -new-name $strSnapSplit[0]\.$strSnapSplit[1]\.$j";
                my @out = runSSHCmd($strSSHCmd);
                if ( $? ne 0 ) {
                    logMsg( $LOG_WARN,
                        "Running '" . $strSSHCmd . "' failed: $?" );
                    logMsg( $LOG_WARN,
"Pleae try again in a few minutes. If issue persists, please contact Microsoft Operations"
                    );
                }
            }
            $j--;
...
sub runRemoveOldestSnapshot {
    ...
    print color('bold cyan');
    logMsg( $LOG_INFO,
"**********************Deleting oldest snapshot list**********************"
    );
    if ( $iteration eq "1" ) {
        logMsg( $LOG_INFO,
"**********************Failure removing oldest snapshot acceptable**********************"
        );
    }
    if ( $iteration eq "2" ) {
        logMsg( $LOG_INFO,
"**********************Failure removing oldest snapshot unacceptable unless clones exist**********************"
        );
    }
    print color('reset');
    for my $i ( 0 .. $#snapshotLocations ) {
        ...
        my $checkSnapshotResult =
          runCheckIfSnapshotExists( $volName, $snapshotName );
        my $lockedSnapshot =
          runCheckSnapshotLocked( $volName, $checkSnapshotResult );
        logMsg( $LOG_INFO,
            "Result: $checkSnapshotResult   Locked: $lockedSnapshot" );
        if ( $checkSnapshotResult eq "0" ) {
            ...
            logMsg( $LOG_INFO,
                  ...
                  . $strSnapshotPrefix . "."
                  . $numKeep
                  . " does not exist on $snapshotLocations[$i][0][0]." );

        }
        elsif ( $lockedSnapshot ne "0" ) {

            logMsg( $LOG_CRIT,
"Snapshot $strSnapshotPrefix\.$numKeep on $volName is locked and will be kept at index: $numKeep"
            );

        }
        else {
            logMsg( $LOG_CRIT,
"Removing oldest snapshot $strSnapshotPrefix\.$numKeep on $snapshotLocations[$i][0][0] on SVM $strSVM ..."
            );
            my $strSSHCmd =
"volume snapshot delete -volume $snapshotLocations[$i][0][0] -snapshot $checkSnapshotResult";
            my @out = runSSHCmd($strSSHCmd);
            if ( $? ne 0 ) {
                logMsg( $LOG_WARN, "Running '" . $strSSHCmd . "' failed: $?" );
                logMsg( $LOG_WARN,
"Please try again in a few minutes. If issue persists, please contact Microsoft Operations"
                );
            }
        }
    }
...
sub runRemoveRecentSnapshot {
    print color('bold cyan');
    logMsg( $LOG_INFO,
"**********************Deleting existing *.recent snapshot**********************"
    );
    logMsg( $LOG_INFO,
"**********************Failures are allowed if *.recent was properly cleaned up last backup**********************"
    );
    print color('reset');
    for my $i ( 0 .. $#snapshotLocations ) {
        ...
        my $volName = $snapshotLocations[$i][0][0];
        my $checkSnapshotResult =
          runCheckIfSnapshotExists( $volName, "$strSnapshotPrefix\.recent" );
        if ( $checkSnapshotResult eq "0" ) {
            logMsg( $LOG_INFO,
"Recent snapshot $strSnapshotPrefix\.recent does not exist on $snapshotLocations[$i][0][0]."
            );
        }
        else {
            # delete the recent snapshot
            logMsg( $LOG_INFO,
"Removing recent snapshot $strSnapshotPrefix\.recent on $snapshotLocations[$i][0][0] on SVM $strSVM ..."
            );
...
sub runRenameRecentSnapshot {
    print color('bold cyan');
    logMsg( $LOG_INFO,
"**********************Renaming *.recent snapshot to *.0**********************"
    );
    print color('reset');
    for my $i ( 0 .. $#snapshotLocations ) {
        logMsg( $LOG_INFO,
"Renaming snapshot $strSnapshotPrefix\.recent to $strSnapshotPrefix\.0 for $snapshotLocations[$i][0][0] on SVM $strSVM ..."
        );
        my $checkSnapshotResult =
          runCheckIfSnapshotExists( $snapshotLocations[$i][0][0],
            "$strSnapshotPrefix\.recent" );
        if ( $checkSnapshotResult eq "0" ) {
            logMsg( $LOG_CRIT,
"Recent snapshot $strSnapshotPrefix\.recent does not exist on $snapshotLocations[$i][0][0]."
            );
            ...
            #logMsg($LOG_INFO,$checkSnapshotResult);
            my $strSSHCmd =
"volume snapshot rename -volume $snapshotLocations[$i][0][0] -snapshot $strSnapSplit[0]\.$strSnapSplit[1]\.recent -new-name $strSnapSplit[0]\.$strSnapSplit[1]\.0";
            my @out = runSSHCmd($strSSHCmd);
            if ( $? ne 0 ) {
                logMsg( $LOG_WARN, "Running '" . $strSSHCmd . "' failed: $?" );
                logMsg( $LOG_WARN,
"$strSnapshotPrefix.recent could not be renamed. This is most likely because of an error rotating snapshot index up for previous snapshots"
                );
            }
            else {
...
sub runCreateHANASnapshot {
    if ( $strBackupType eq "hana" ) {
        ...
        if ( $intMDC eq 1 and $strHANAVersion eq 1 ) {
            logMsg( $LOG_CRIT,
                "MDC Architecture and HANA Version 1 Detected." );
            logMsg( $LOG_WARN,
"SAP does not support snapshots in a MDC environment for the currently detected release. Snapshots are supported as a non-MDC install on the detected release."
            );
            runExit($exitWarn);
        }
        ...
        {
            logMsg( $LOG_WARN,
                "MDC architecture and HANA Version 2 SP 00 Detected." );
            logMsg( $LOG_WARN,
"SAP does not support snapshots in a MDC environment for the currently detected release. Snapshots are supported as a non-MDC install on the detected release."
            );
            runExit($exitWarn);
        ...
        logMsg( $LOG_CRIT,
            "Creating the HANA snapshot with command: \"$strHANACreateCmd\" ..."
        );
        my @out = runShellCmd($strHANACreateCmd);
        if ( $? ne 0 ) {
            logMsg( $LOG_WARN,
                    "HANA snapshot creation command '"
                  . $strHANACreateCmd
                  . "' failed: $?" );
            if (    $intMDC eq 1
            ...
            {
                logMsg( $LOG_WARN,
                    "Please ensure this is a single tenant system." );
                logMsg( $LOG_WARN,
"HANA Snapshot for multitenant system is not currently supported by SAP."
                );
                logMsg( $LOG_WARN,
                    "Please verify that a HANA Snapshot is not already open." );
...
sub runCheckHANASnapshotStatus {
    ...
    if ( $strBackupType eq "hana" ) {
        ...
        print color('bold cyan');
        logMsg( $LOG_CRIT,
"**********************Checking for HANA snapshot and obtaining ID**********************"
        );
        print color('reset');
        ...
        if ( $strHANAVersion eq 1 ) {
            logMsg( $LOG_CRIT,
"Checking HANA snapshot status with command: \"$strHANABackupIDRequestV1\" ..."
            );
        ...
        if ( $strHANAVersion eq 2 ) {
            logMsg( $LOG_CRIT,
"Checking HANA snapshot status with command: \"$strHANABackupIDRequestV2\" ..."
            );
        ...
        if ( $? ne 0 ) {
            logMsg( $LOG_WARN,
                  ...
                  . "' failed: $?" );
            logMsg( $LOG_WARN,
"Most likely cause of HANA snapshot failure is a previous HANA snapshot was never closed or deleted."
            );
            logMsg( $LOG_WARN,
                "Please investigate backup status of $strHANASID and try again."
            );
            logMsg( $LOG_WARN,
"******************Exiting Script*******************************"
            );
            ...
            print color('bold green');
            logMsg( $LOG_CRIT,
                "HANA snapshot backupid discovered:  $strHANABackupID." );
            print color('reset');
        }
    }
}
    ...
    if ( $strBackupType eq "hana" ) {
        ...
        if ( $strHANAVersion eq 2 ) {
            $strHANACloseCmdV2 =
              ...
              . ' -d SYSTEMDB -U '
              . $strHANAAdmin
              . ' "backup data for full system close snapshot backup_id '
              . $strHANABackupID
              . ' SUCCESSFUL '
              . $hanaSnapshotSuccess . qq(");
        ...
        logMsg( $LOG_CRIT,
            "Closing the HANA snapshot with command: \"$strHANACloseCmd\" ..."
        );
        my @out = runShellCmd($strHANACloseCmd);
        if ( $? ne 0 ) {
            logMsg( $LOG_WARN,
                    "HANA snapshot deletion command '"
                  . $strHANACloseCmd
                  . "' failed: $?" );
            logMsg( $LOG_WARN,
"Please verify backup status of $strHANASID and close snapshot if necessary"
            );
            logMsg( $LOG_WARN,
"The command listed above may be used to close the snapshot.  If this command fails, please contact Microsoft Operations"
            );
        }
        else {
            print color('bold green');
...
sub runCreateStorageSnapshot {
    ...
    print color('reset');
    my $hostname = hostname;    # used as part of Volume Name
    $hostname =~ s/\-/_/g
      ;    # convert hyphen to underscore as Volume Names can't have a hyphen
    ...
    for my $i ( 0 .. $#snapshotLocations ) {
        ...
        $strSSHCmd = "";
        logMsg( $LOG_CRIT,
"Verify snapshot $strSnapshotPrefix\.recent creation for $snapshotLocations[$i][0][0] ..."
        );
...
#storage command necessary to create storage snapshot, others items to include: snapmirror-label matching snapshot type/frequency and HANA snapshot backup id matching as comment
        ...
        if ( $strBackupType eq "hana" ) {
            ...
            my $strDIRLocationCheck =
              "/hana/data/$strHANASIDUC/mnt00001/hdb00001";
            my $strDIRLocationCheck2 =
              "/hana/data/$strHANASIDUC/mnt00001/mnt00001/hdb00001";
            my $strSnapshotLocation =
"/hana/data/$strHANASIDUC/mnt00001/hdb00001/snapshot_databackup_0_1";
            my $strSnapshotLocation2 =
"/hana/data/$strHANASIDUC/mnt00001/mnt00001/hdb00001/snapshot_databackup_0_1";
            if ( -e $strDIRLocationCheck ) {
                logMsg( $LOG_CRIT,
                    "Checking for snapshot at $strSnapshotLocation" );
            }
            elsif ( -e $strDIRLocationCheck2 ) {
                logMsg( $LOG_CRIT,
                    "Checking for snapshot at $strSnapshotLocation2" );
            }
            else {
                logMsg( $LOG_WARN,
"Unable to find suitable directory to check for location of HANA snapshot"
                );
              ...
              )
            {
                ...
                $strSSHCmd =
"volume snapshot create -volume $volName -snapshot $strSnapshotPrefix\.$date\.recent -snapmirror-label $strSnapshotCustomerLabel -comment $strHANABackupID";
                push( @snapshotCreated, $volName );
            }
            elsif (
                ( ( -e $strSnapshotLocation ) or ( -e $strSnapshotLocation2 ) )
                and ( $volName !~ m/$volDataNoHSR/ ) )
            {
                print color('bold magenta');
                logMsg( $LOG_CRIT,
"Skipping $volName as it was determined to be a non-active HSR Volume."
                );
                logMsg( $LOG_CRIT,
"If this volume is not part of an HSR setup, please contact Microsoft Operations for assistance."
                );
                print color('reset');
                next;
            }
                ...
                logMsg( $LOG_WARN,
"Please verify HANA snapshot status in Backup Catalog in HANA Studio."
                );
                runHANACloseSnapshot();
                runExit($exitWarn);
            }
        ...
        if ( $strBackupType eq "logs" ) {
                ...
                or ( $volName =~ m/$volLogNoHSR/ ) )
            {
                ...
                $strSSHCmd =
"volume snapshot create -volume $volName -snapshot $strSnapshotPrefix\.$date\.recent -snapmirror-label $strSnapshotCustomerLabel";
                push( @snapshotCreated, $volName );
            }
            else {
                print color('bold magenta');
                logMsg( $LOG_CRIT,
"Skipping $volName as it was determined to be a non-active HSR Log Backups Volume."
                );
                logMsg( $LOG_CRIT,
"If this volume is not part of an HSR setup, please contact Microsoft Operations for assistance."
                );
...
"volume snapshot create -volume $volName -snapshot $strSnapshotPrefix\.$date\.recent -snapmirror-label $strSnapshotCustomerLabel";
        ...
        if ( $? ne 0 ) {
            logMsg( $LOG_WARN,
                "Snapshot creation command '" . $strSSHCmd . "' failed: $?" );
            logMsg( $LOG_WARN,
"Please try script again in a few minutes. If problem still persists, contact Microsoft Operations"
            );
        }
        else {
            print color('bold green');
            logMsg( $LOG_CRIT,
                "Snapshot created successfully for volume $volName." );
            print color('reset');
        }
    }
}
...
"**********************Adding list of snapshots to volume list**********************"
    ...
    logMsg( $LOG_INFO,
"Collecting set of snapshots for each volume hosting HANA matching pattern *$strHANASID* ..."
    );
    foreach my $volName (@volLocations) {
        my $j = 0;
        $snapshotLocations[$i][0][0] = $volName;
        my $strSSHDiagCmd =
          "volume snapshot show -volume $volName -fields snapshot,record-owner";
        my @out = runSSHDiagCmd($strSSHDiagCmd);
        if ( $? ne 0 ) {
            logMsg( $LOG_WARN, "Running '" . $strSSHDiagCmd . "' failed: $?" );
            logMsg( $LOG_WARN,
"Please verify that the testStorageSnapshot script was executed and not executed the removeTestStorageSnapshot.pl script yet. It is a required script before executing this script."
            );
            logMsg( $LOG_WARN,
"If testStorageSnapshot was successful, please try again in a few minutes. "
            );
            logMsg( $LOG_WARN,
"If issue persists, please contact Microsoft Operations for support"
            );
            runExit($exitWarn);
        }
        my $listnum = 0;
...
"**********************Rotating snapshot numbering to allow new snapshot**********************"
  ...
  # let's go through all the Filer and volume paths, rotating snapshots for each
    for my $i ( 0 .. $#snapshotLocations ) {
        ...
        # iterate through all the snapshots
        logMsg( $LOG_INFO,
"Rotating older snapshots named $strSnapshotPrefix.$j on $snapshotLocations[$i][0][0] ..."
        );

        while ( $j >= $numKeep ) {

            $checkSnapshotResult =
              runCheckIfSnapshotExists( $volName, "$strSnapshotPrefix\.$j" );
            ...
            if ( $checkSnapshotResult ne "0" ) {
                ...
                if ( $strSnapSplit[1] ne "" ) {
                    ...
                    my $checkSnapshotAge =
                      runCheckSnapshotAge($snapshotTimeStamp);
                    my $lockedSnapshot =
                      runCheckSnapshotLocked( $volName, $snapshotName );
                    if ( $checkSnapshotAge and !$lockedSnapshot ) {
                        my $strSSHCmd =
"volume snapshot delete -volume $volName -snapshot $snapshotName";
                        my @out = runSSHCmd($strSSHCmd);
                        if ( $? ne 0 ) {
...
"Please try again in a few minutes. If issue persists, please contact Microsoft Operations."
                        ...
                        print color('bold red');
                        logMsg( $LOG_WARN,
"Snapshot aged less than 10 minutes... cannot delete. Stopping execution."
                        );
                        logMsg( $LOG_WARN,
"Snapshots are checked for replication every 10 minutes.  You cannot delete a snapshot that the latest replication is based upon."
                        );
                        logMsg( $LOG_WARN,
"Either wait 10 minutes so that the replication is no longer based on the previously created snapshot or contact Microsoft Operations."
                        );
                        print color('reset');
                        runExit($exitWarn);
                    }
                        ...
                        print color('bold cyan');
                        logMsg( $LOG_CRIT,
"Snapshot cloned in DR location.  Clone must be removed before snapshot is deleted. Rotating Snapshot to higher value than $numKeep temporarily."
                        );
                        print color('reset');
                        my $z = $j + 1;
                        my $strSSHCmd =
"volume snapshot rename -volume $volName -snapshot $strSnapSplit[0]\.$strSnapSplit[1]\.$j -new-name $strSnapSplit[0]\.$strSnapSplit[1]\.$z";
                        my @out = runSSHCmd($strSSHCmd);
                        if ( $? ne 0 ) {
                            logMsg( $LOG_WARN,
                                "Running '" . $strSSHCmd . "' failed: $?" );
...
sub runCheckSnapshotLocked {
    ...
    print color('bold cyan');
    logMsg( $LOG_INFO,
        "Checking status of volume $volName and snapshot $snapshotName" );
    print color('reset');
    ...
    while ( $snapshotLocations[$i][$j][0] ne $volName ) {
        logMsg( $LOG_INFO,
            "volume: $volName searched: " . $snapshotLocations[$i][$j][0] );
        $i++;
...
#now that we have found the matching volume and snapshot, need to verify status of snapshot
    if ( $snapshotLocations[$i][$j][1] =~ m/snapmirror/ ) {
        ...
        print color('bold cyan');
        logMsg( $LOG_INFO,
"Snapshot $snapshotName of volume $volName has record owner of $snapshotLocations[$i][$j][1]"
        );
        print color('reset');
        return 1;
    }
        ...
        print color('bold cyan');
        logMsg( $LOG_INFO,
            "Snapshot $snapshotName of volume $volName has no record owner" );
        print color('reset');
        return 0;
    }
}
...
sub runRotateLockedSnapshotsDown {
    ...
    print color('bold cyan');
    logMsg( $LOG_INFO,
"**********************Rotating locked snapshots down to minimize number**********************"
    );
    print color('reset');
    my $rotateIndex;
    my $j;
...
#to get right index, verifying whether snapshot corresponding to $numKeep was removed or not.
        $checkSnapshotResult =
          runCheckIfSnapshotExists( $volName, "$strSnapshotPrefix\.$numKeep" );
        logMsg( $LOG_INFO,
            "Check Snapshot result for $numKeep: $checkSnapshotResult" );
        if ( $checkSnapshotResult ne "0" ) {
            logMsg( $LOG_INFO,
                    "Snapshot "
                  . $strSnapshotPrefix . "."
        ...
        # iterate through all the snapshots
        logMsg( $LOG_INFO,
            "Rotating snapshots named $strSnapshotPrefix.# on $volName ..." );
        ...
        while ( $j <= $numSnapshotCount ) {
            ...
            $checkSnapshotResult =
              runCheckIfSnapshotExists( $volName, "$strSnapshotPrefix\.$j" );
            logMsg( $LOG_INFO,
                "Check Snapshot result for $j: $checkSnapshotResult" );
            if ( $checkSnapshotResult ne "0" ) {
                Time::HiRes::sleep( 0.5 + rand(0.5) );
                my @strSnapSplit = split( /\./, $checkSnapshotResult );
                my $strSSHCmd =
"volume snapshot rename -volume $volName -snapshot $strSnapSplit[0]\.$strSnapSplit[1]\.$j -new-name $strSnapSplit[0]\.$strSnapSplit[1]\.$k";
                my @out = runSSHCmd($strSSHCmd);
                if ( $? ne 0 ) {
                    logMsg( $LOG_WARN,
                        "Running '" . $strSSHCmd . "' failed: $?" );
                    logMSg( $LOG_WARN,
"Unable to rename snapshot. Either the snapshot name already exists or there was an error communicating with the storage."
                    );
                    logMsg( $LOG_WARN,
"Please try again in a few minutes. If the problem persists, contact Microsoft Operations"
                    );
                }
                $k++;
            }
...
sub runCountSnapshotPrefix {
    ...
    print color('bold cyan');
    logMsg( $LOG_INFO,
"**********************Counting number of Snapshots that match customer provided snapshot prefix**********************"
    );
    print color('reset');
    ...
    for my $i ( 0 .. $#snapshotLocations ) {
        ...
        for my $j ( 1 .. $#{$aref} ) {
            ...
            logMsg( $LOG_INFO,
"snapshot: $snapshotLocations[$i][$j][0] snapshot prefix: $strSnapSplit[0] number: $strSnapSplit[2] prefix: $strSnapshotPrefix"
            );
            if ( $strSnapSplit[0] eq $strSnapshotPrefix ) {
                logMsg( $LOG_INFO, "Snapshot Matched" );
                logMsg( $LOG_INFO,
                    "index: $strSnapSplit[2]   count: $numSnapshotCount " );
    ...
    print color('bold cyan');
    logMsg( $LOG_INFO,
"**********************Snapshot Count: $numSnapshotCount**********************"
    );
    print color('reset');
...
sub runPrintFile {
    ...
    print color('bold green');
    logMsg( $LOG_CRIT,
        "Log file created at " . $existingdir . "/" . $outputFilename );
    print color('reset');
...
if (    ( $strBackupType eq "hana" or $strBackupType eq "logs" )
    and
    ( !defined( $ARGV[1] ) or !defined( $ARGV[2] ) or !defined( $ARGV[3] ) ) )
{

    logMsg( $LOG_WARN,
"Please enter argument as either (hana or logs) <Customer Snapshot Label> <frequency> <retention> (verbose)"
    );
    runExit($exitWarn);
}
...
{
    logMsg( $LOG_WARN,
"Please enter argument as either boot <TYPEI or TYPEII> <Customer Snapshot Label> <frequency> <retention> (verbose)"
    );
    runExit($exitWarn);
}
...
if ( $strBackupType eq "hana" or $strBackupType eq "logs" ) {
    ...
    #set up our variables for each SID based on backup type and HANA enviroment.
            ...
            logMsg( $LOG_CRIT,
                "Executing " . $strBackupType . " Snapshots for $strHANASID" );
            print color('reset');
            ...
            if ( not $strSnapshotPrefix =~ /^[a-zA-Z0-9_-]*$/ ) {
                ...
                logMsg( $LOG_WARN,
                      ...
                      . "' not allowed" );
                logMsg( $LOG_WARN,
"Please use alpha-numeric (A-Za-z0-9), - (dash), and _ underscore characters only"
                );
                runExit($exitWarn);
            }
            ...
            if ( not $numKeep =~ /^[0-9]*$/ ) {
                ...
                #only want Alpha-numeric, "-", & "_" characters in the prefix
                logMsg( $LOG_WARN,
                        "Retention Value must be number only."
                      . $numKeep
                      . " is not allowed" );
                runExit($exitWarn);
            }
            if ( $numKeep <= 0 or $numKeep > 250 ) {
                ...
                logMsg( $LOG_WARN,
"A Customer provided retention value less than 1 or greater than 250 is not allowed"
                );
                runExit($exitWarn);
            }

            if (    ( $strSnapshotCustomerLabel ne "15min" )
                and ( $strSnapshotCustomerLabel ne "3min" ) )
            {
#current accepted snapmirror labels (frequency) are 15min for data, 3min for log backups
                logMsg( $LOG_WARN,
"A frequency of $strSnapshotCustomerLabel is not an acceptable frequency."
                );
                logMsg( $LOG_WARN,
                    "For HANA backups and boot backups, please use 15min" );
            ...
            $strHANAStatusCmdV2 =
              ...
              . ' -d SYSTEMDB -U '
              . $strHANAAdmin . ' "\s"';
            $strHANACreateSnapCmdV1 =
                './hdbsql -n '
              . $strPrimaryHANAServerIPAddress . ' -i '
              . $strHANANumInstance . ' -U '
              . $strHANAAdmin
              . ' "backup data create snapshot"';
            $strHANACreateSnapCmdV2 =
                './hdbsql -n '
              . $strPrimaryHANAServerIPAddress . ' -i '
              . $strHANANumInstance
              . ' -d SYSTEMDB -U '
            ...
            $strHANABackupIDRequestV1 =
                './hdbsql -n '
              . $strPrimaryHANAServerIPAddress . ' -i '
              . $strHANANumInstance . ' -U '
              . $strHANAAdmin . ' "'
              . $strHANAIDRequestString . '"';
            $strHANABackupIDRequestV2 =
                './hdbsql -n '
              . $strPrimaryHANAServerIPAddress . ' -i '
              . $strHANANumInstance
              . ' -d SYSTEMDB -U '
            ...
            logMsg( $LOG_CRIT,
                "No data entered for SID" . ( $i + 1 ) . "  Skipping!!!" );
            next;
        }
