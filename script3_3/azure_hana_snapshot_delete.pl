...
my $LOG_INPUT = 4
  ; #displays output to both console and file always, does not include new line command at end of output.
...
my @HANASnapshotLocations
  ;    #contains list of volumes and snapshots that match backup id
...
my $strSnapshotName
  ;    #snapshot name if customer selects deletion by voluem and snapshot
my $strVolumeLoc
  ;    #volume location if customer selects deletion by voluem and snapshot
my $boolBackupidFound
  ; #proceeds with backupid deletion if backupid is found on a snapshot detail list
...
my $strSecondaryHANAServerName
  ;    #Customer provided IP Address or Qualified Name of Seconary HANA Server.
my $strSecondaryHANAServerIPAddress
  ;    #Customer provided IP address of Secondary HANA Server
...
my $HSR            = 0
  ; #used within only scripts, otherwise flagged to zero. **not used in this script**
...
sub runDeleteSnapshot {
    ...
    my $strProceedSnapshot =
"You have requested to delete snapshot $strSnapshotName from volume $strVolumeLoc. Any data that exists only on this snapshot is lost forever. Do you wish to proceed (yes/no)?   ";
    ...
    logMsg( $LOG_CRIT,
"*********************Deleting Snapshot $strSnapshotName from Volume $strVolumeLoc**********************"
    );

    my $strSSHCmd =
"volume snapshot show -volume $strVolumeLoc -snapshot $strSnapshotName -fields create-time";
    my @out = runSSHCmd($strSSHCmd);
    if ( $? ne 0 ) {
        logMsg( $LOG_WARN, "Running '" . $strSSHCmd . "' failed: $?" );
        logMsg( $LOG_WARN,
"Please verify correct volume: $strVolumeLoc and snapshot: $strSnapshotName were provided"
        );
        logMsg( $LOG_WARN,
            "Otherwise, please wait a few minutes and try again" );
        logMsg( $LOG_WARN,
"If issue persists, please contact Microsoft Operations for assistance"
        );
    }
    ...
    if ($checkSnapshotAge) {
        my $strSSHCmd =
"volume snapshot delete -volume $strVolumeLoc -snapshot $strSnapshotName";
        my @out = runSSHCmd($strSSHCmd);
        if ( $? ne 0 ) {
            logMsg( $LOG_WARN, "Running '" . $strSSHCmd . "' failed: $?" );
...
"If issue persists, Please contact Microsoft Operations for assistance"
            ...
            print color('bold green');
            logMsg( $LOG_CRIT,
"Snapshot $strSnapshotName of volume $strVolumeLoc was successfully deleted"
            );
            print color('reset');
        }
        ...
        logMsg( $LOG_WARN,
"$strSnapshotName is aged less than 10 minutes... cannot delete due to potential replica interference. Stopping execution."
        );
        runExit($exitWarn);
    }
}
...
sub runDeleteHANASnapshot {
    my $strBackupidInput =
"Please enter the backup id of the HANA Storage Snapshot you wish to delete: ";
    print color('bold cyan');
    ...
    my $strProceedHANASnapshot =
"You have requested to delete all snapshots associated with HANA Backup ID $strBackupid. Any data that exists solely on these snapshots are lost forever. Do you wish to proceed (yes/no)?   ";
    print color('bold cyan');
    ...
    if ($boolBackupidFound) {
        for my $x ( 0 .. $#HANASnapshotLocations ) {
            my $strSSHCmd =
"volume snapshot show -volume $HANASnapshotLocations[$x][0] -snapshot $HANASnapshotLocations[$x][1] -fields create-time";
            my @out = runSSHCmd($strSSHCmd);
            if ( $? ne 0 ) {
                logMsg( $LOG_WARN, "Running '" . $strSSHCmd . "' failed: $?" );
                logMsg( $LOG_WARN, "Please try again in a few minutes." );
...
"If issue persists, Please contact Microsoft Operations for assistance"
            ...
            my $strSnapshotTime = $strSubArr[3];
            logMsg( $LOG_INFO,
"Checking time stamp for snapshot $HANASnapshotLocations[$x][1] of volume $HANASnapshotLocations[$x][0]"
            );
            my $checkSnapshotAge = runCheckSnapshotAge($strSnapshotTime);
            if ($checkSnapshotAge) {
                my $strSSHCmd =
"volume snapshot delete -volume $HANASnapshotLocations[$x][0] -snapshot $HANASnapshotLocations[$x][1]";
                my @out = runSSHCmd($strSSHCmd);
                if ( $? ne 0 ) {
                    logMsg( $LOG_WARN,
                    ...
                    logMsg( $LOG_WARN,
"If issues persists, Please contact Microsoft Operations for assistance."
                    );
                }
                else {
                    print color('bold green');
                    logMsg( $LOG_CRIT,
"Snapshot $HANASnapshotLocations[$x][1] of volume $HANASnapshotLocations[$x][0] was successfully deleted"
                    );
                    print color('reset');
                }
                ...
                logMsg( $LOG_WARN,
"$HANASnapshotLocations[$x][1] is aged less than 10 minutes... cannot delete due to potential replica interference. Stopping execution."
                );
                runExit($ERR_WARN);
            }
        ...
        print color('bold red');
        logMsg( $LOG_WARN,
"No snapshots found that correspond to HANA Backup id $strBackupid.  Please double-check the HANA Backup ID as specified in HANA Studio and try again.  If you feel you are reaching this message in error, please open a ticket with MS Operations for additional support."
        );
        runExit($ERR_WARN);
    }
}
...
"**********************Getting list of volumes****************************"
    ...
    my $strSSHCmd =
        "volume show -volume *"
      . $strHANAEnteredSID
      . "* -volume *data* | *shared* -volume !*dp* | !*xdp* -type RW -fields volume";
...
"**********************Adding list of snapshots to volume list**********************"
    ...
    foreach my $volName (@volLocations) {
        ...
        my $strSSHCmd =
          "volume snapshot show -volume $volName -fields snapshot";
        my @out = runSSHCmd($strSSHCmd);
        if ( $? ne 0 ) {
            logMsg( $LOG_INFO, "Running '" . $strSSHCmd . "' failed: $?" );
            logMsg( $LOG_INFO,
                "Possible reason: No snapshots were found for volume: $volName."
            );
            next;
        }
        my $listnum = 0;
        ...
        foreach my $k ( 0 ... $count ) {
            ...
            if ( index( $strSub, "snapmirror" ) == -1 ) {
                ...
                #print "$strSub\n";
                logMsg( $LOG_INFO,
                    "Snapshot $strSub added to $snapshotLocations[$i][0][0]" );
...
sub runGetSnapshotDetailsBySnapshot {
    logMsg( $LOG_INFO,
"**********************Adding snapshot details to snapshot list**********************"
    );
    ...
    for my $x ( 0 .. $#snapshotLocations ) {
        ...
        for my $y ( 1 .. $#{$aref} ) {
            ...
            my $strSSHCmd =
"volume snapshot show -volume $snapshotLocations[$x][0][0] -snapshot $snapshotLocations[$x][$y][0] -fields snapshot, comment";
            my @out = runSSHCmd($strSSHCmd);
        ...
        for my $j ( 0 .. $#{$aref} ) {
            if ( defined( $snapshotLocations[$i][$j][1] ) ) {
                if ( $snapshotLocations[$i][$j][1] eq $strBackupid ) {
                    ...
                    logMsg( $LOG_CRIT,
"Adding Snapshot $HANASnapshotLocations[$k][1] from volume $HANASnapshotLocations[$k][0]"
                    );
                    $k++;
                }
            }
...
sub runCheckSnapshotAge {
    ...
    my $t =
      Time::Piece->strptime( $snapshotTimeStamp, "%a %b %d %H:%M:%S %Y" ); # UTC
    my $tNum = str2time($t);

    my $currentTime = gmtime->strftime("%a %b %e %H:%M:%S %Y");            # UTC
    my $currentT =
      Time::Piece->strptime( $currentTime, "%a %b %d %H:%M:%S %Y" );
    ...
    if ( ( str2time($currentT) - str2time($t) ) > 600 ) {
        logMsg( $LOG_INFO,
            "Time threshold passed.  Okay to proceed in snapshot deletion" );
        return 1;
    }
    else {
        return 0;
...
logMsg( $LOG_CRIT,
    "Executing Azure HANA Snapshot Delete Script, Version $version" );

if ( !defined $ARGV[0] ) {

    logMsg( $LOG_WARN,
"Please enter HANA Instance pertaining to the snapshot you wish to delete."
    );
    runExit($ERR_WARN);
}
...
while ( $arrCustomerDetails[$i][0] ne $strHANAEnteredSID ) {
    ...
    if ( $i eq $numSID ) {
        ...
        logMsg( $LOG_WARN,
"The Entered SID was not found within the HANABackupCustomerDetails.txt file.  Please double-check that file."
        );
        print color('bold red');
        runExit($ERR_WARN);
